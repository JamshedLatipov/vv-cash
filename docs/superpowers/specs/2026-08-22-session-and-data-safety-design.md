...
**`ClearUnsyncedDocuments` удаляется целиком.** Кнопка появилась, чтобы расчистить
навечно застрявшую очередь. Основной случай — сервер отверг документ по существу —
уже решён правильным способом, через `MarkDocumentRejectedAsync`: такой документ
выходит из ротации сам и остаётся на диске для бэк-офиса. Кнопка же осталась
способом потерять выручку в одно нажатие, с экрана, доступного до аутентификации.

**Честно про остаток.** «Решено правильно» покрывает не все случаи. Ревью Task 7
прошло по `SyncOfflineDocumentsAsync` и нашло три пути, где очередь не рассосётся
никогда, потому что `IsFinalRefusal` их отказом не считает:

- постоянный 5xx — намеренно ретраится вечно («сломался сервер, а не документ»);
- payload, который больше не десериализуется, — ловится общим `catch` и до сети
  не доходит вовсе (реалистичный сценарий: обновление приложения поменяло форму
  `DocumentRequest`, пока касса была офлайн со старой записью в очереди);
- 4xx не в конверте приложения — HTML-страница ошибки от прокси, `CarriesRefusalEnvelope`
  её не разбирает.

Во всех трёх бейдж непроведённых горит бесконечно, а после удаления кнопки в
приложении не остаётся ни одного действия — ни безопасного, ни разрушительного, —
которое бы это сняло: только правка SQLite-файла на диске конкретной кассы.

Это принято сознательно. Гэп предсуществующий, а удалённая кнопка лечила его
стиранием *всех* записей о взятых деньгах — лекарство хуже болезни. Безопасная
замена (адресное снятие одного документа через тот же `MarkDocumentRejectedAsync`,
за подтверждением и не с экрана логина) заведена отдельной задачей и в этот батч
не входит.
