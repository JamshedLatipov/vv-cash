...
> **Здесь план разошёлся с кодом, и код прав.** Код-ревью Task 12 показало, что
> `bool` склеивает «сервер недоступен» и «сервер отказал в этом заказе», а реакции
> у них противоположные: на первом досыл останавливается целиком, на втором —
> откладывает запись и идёт дальше. С `bool` один заказ, который сервер не примет
> никогда, встаёт в голове буфера и держит все последующие навсегда. Поэтому
> `PostOrderAsync` возвращает `PostOrderResult { Sent, Refused, Unreachable }`.
> Дальнейшие сниппеты плана, где ещё стоит `bool`, читать с этой поправкой —
> актуальны файлы `IQueueTransport.cs` и `HttpQueueTransport.cs`, а не они.
...
```xml
    <!-- Kestrel для локального сервера очереди. Publish self-contained, значит
         рантайм ASP.NET Core уезжает внутрь сборки: +27,9 МБ на win-x64 и
         +26,2 МБ на win-x86, со 113 до 141 МБ — измерено сравнением публикаций
         до и после, а не оценкой. Это цена каждого автообновления на каждой
         кассе, и она принята сознательно: HttpListener из BCL не весит ничего,
         но требует netsh http add urlacl под конкретный порт при установке, а
         смена порта в настройках после этого молча перестаёт работать. -->
