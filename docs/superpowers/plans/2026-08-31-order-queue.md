...
`PrintExchangeReceiptAsync` остаются на `_printers` целиком: пречек, денежный
ящик, возврат и обмен ролями не делятся — их печатает тот же аппарат, что и чек,
и вводить для них четвёртую роль незачем.

Замечание: `PrintReturnReceiptAsync` и `PrintExchangeReceiptAsync` на аппарате,
у которого роли `Ticket`-only, теперь напечатают возврат. Это осознанно — точка
с одним талонным принтером и без чекового и раньше печатала возвраты на нём.
