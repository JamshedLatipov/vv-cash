...
надо — оно обеспечено в самой `HasNoResults`. При применении выяснилось, что
центрированная плашка ложится поверх сохранённого списка результатов, поэтому
ошибка сделана полосой сверху, а не оверлеем (`28e1ea8`).

**Проверять разметку сборкой нельзя.** `src/VvCash/VvCash.csproj` ставит
`AvaloniaUseCompiledBindingsByDefault=false`, `x:CompileBindings` не выставлен
нигде — привязки рефлексивные. Путь, называющий несуществующее свойство,
собирается чисто и молча ничего не рисует; отсутствующий ключ `I18nService`
рисуется как литерал `[Ключ]`. Сборка проверяет только ресурсы, `x:Static`,
значения enum и синтаксис. План этого не учитывал и обещал, что опечатки
поймает Step 3.

**Task 7 (сверх плана) — провал регистрации молча закрывал окно.**
`SubmitAsync` закрывал окно с `null` и при отказе сервера, то есть неотличимо
от отмены: кассир жал «Сохранить», окно исчезало, введённые данные пропадали,
объяснения не было. Теперь окно остаётся открытым с заполненной формой и
показывает `CustomerCreateFailed`. Ключ добавлен в пять локалей вместе с
остальными (`75819b5`).

**Финальное ревью, последний раунд (`4e28fbd`).** Пять правок:
подсказка «создайте нового клиента» в пустом состоянии не гасилась флагом —
UI звал к действию, которого на экране нет; `return allResults` в
`CounterpartyService` оказался несущей веткой (сервер отвечает на пустой
результат через `response.EmptyList`, а тот не несёт поля `status`, поэтому
ответ не подходит ни под массив, ни под конверт) — заменён на явный пустой
список с комментарием и четвёртым тестом; `SelectedCounterparty` теперь
снимается новым поиском явно, а не в расчёте на обратную запись из `ListBox`;
`SubmitAsync` чистит `ErrorMessage` на входе; оставшийся жёсткий каст в
`OpenCustomerSearch` заменён на `as`.

Итог по ветке: **429 тестов**, чистая сборка, три предсуществующих
предупреждения.
