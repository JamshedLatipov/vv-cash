...
```csharp
    ...
    /// каждым значит однажды пропустить четвёртый. Как раз он и есть та точка:
    /// единственная, что собиралась инлайном, а не через билдер, и потому
    /// единственная, до которой не доставал ни один тест. Его молчаливый откат на
...
**Само по себе свойство шва ещё не образует, и часть 2 обязана его достроить.** `_printers` приватный, `InternalsVisibleTo` в проекте нет — значит тест из `VvCash.Tests` физически не дойдёт от композита до принтера, чтобы прочитать `.CodePage`. Пока часть 2 не откроет снимок принтеров наружу, `CodePage` остаётся публичным членом без единого потребителя, а его собственный doc обещает тест, который написать нельзя.

Шаг 3 ниже всё равно заменяет `_printers` на снимок, присваиваемый одним движением, — снимок это естественная поверхность только для чтения. Открыть его (`internal` + `InternalsVisibleTo`, либо публичное `IReadOnlyList`) и написать тест «принтер построен с той страницей, что задана в настройках» — обязательная часть шага 3, а не пожелание. После этого grep-гейт из Task 6 снимается.

Альтернатива, если открывать список не захочется: проверять не геттер, а поведение — собрать композит поверх поддельного `ISettingsService` с `CodePageId = "CP1251"` и убедиться, что чек начинается на `ESC t 46`, а не `ESC t 17`. Это лучше по существу, потому что проверяет доезд таблицы до бумаги. Упирается в ту же приватность списка. **Если часть 2 пойдёт этим путём — `CodePage` из шага 0d надо удалить, а не оставлять мёртвой поверхностью.**
...
**Транспорт в этом тесте не должен быть сетевым, и это выяснено дорого.** Первая редакция печатала на закрытый порт loopback и не воспроизвела гонку **ни разу** за три прогона по 6m47s — то есть откат исправления оставил бы её зелёной. Причина машинная: отказ в соединении на закрытый порт `127.0.0.1` здесь занимает около **2180 мс** вместо долей миллисекунды, поэтому цикл перенастройки успевает отработать все итерации, пока цикл печати стоит на первой, и окно перекрытия схлопывается в ноль. Без сокета та же гонка ловится с первой попытки за десятки миллисекунд, а полный набор тестов остаётся секундным вместо семиминутного.

Способ убрать ввод-вывод: значение `PrinterConnectionType` вне диапазона попадает в ветку `default:` метода `SendAsync` и возвращается немедленно. Приём законный — предмет теста здесь состав списка, а не транспорт, — но в комментарии его надо назвать намеренным, иначе он читается как опечатка.

Проявление тоже стоит записать в комментарии: под .NET 10 это `NullReferenceException` из `ListSelectIterator.Fill` (рваное чтение массива-подложки, пока его чистят), а не классический `InvalidOperationException: Collection was modified`. Иначе следующий человек с другой формой исключения решит, что тест протух.
...
**Две заметки к формулировкам, а не к коду.**

`InvalidOperationException`, ради которого шаг 0 научил `SetStatus` глотать исключение подписчика, после снимка исчезнет: список больше не правится на месте. То есть глотание превращается из починки в защиту в глубину. Doc у `SetStatus` гонку не называет, поэтому править его не придётся — но в сообщении коммита это стоит проговорить, иначе следующий читатель решит, что защита избыточна, и снимет её.

И `try`/`catch` вокруг `Invoke` не даёт исключению дойти до метода печати, но остальные подписчики после броска всё равно не вызовутся — так устроен `Delegate.Invoke`. Сегодня подписчик ровно один (`OnPrinterStatusChanged`), так что это теория. Если часть 2 добавит второго, распространение статуса начнёт молча обрываться на первом упавшем; лечится обходом `GetInvocationList()` с `catch` на каждом.
...
- [ ] **Step 4b: Две правки в `VfdDisplayService`, обязательные перед включением службы боевой**

До этой задачи VFD не был зарегистрирован нигде, поэтому оба дефекта ничего не стоили. Step 6 делает его боевым, и с этого момента они начинают стоить.
...
40 байт на 9600 бод — это около 42 мс провода, так что полсекунды более чем щедро. Без этой строки кнопка «Проверить дисплей» из Task 12 будет не отчитываться, а зависать — то есть не выполнит того, ради чего заведена.
...
## Долг, отложенный сознательно

Не дефекты — решения отложить, чтобы не плодить правки в файлах, которые батч и так открывает многократно. Записаны, чтобы не потерялись.

- **`"0.###"` живёт четырьмя литералами** (`EscPosPrinterService` ×3, `CartItem` ×1), `"0.######"` — одним. Task 8 убрал дублирование *алгоритма*, но не *точности*: решение печатать четыре знака после запятой придётся искать в четырёх местах — ровно тот класс расхождения корзины и чека, ради которого хелпер и заводился. Лечится двумя константами на `QuantityFormat` (`Quantity = "0.###"`, `UnitAmount = "0.######"`), которые заодно называют смысл точности, а не только значение. Отдельным проходом после батча.
- **`QuantityFormat.Display` игнорирует `fractionFormat` на целочисленной ветке.** Для обоих сегодняшних форматов ветка поведенчески избыточна — `(2.0m).ToString("0.###")` и так даёт `"2"`. Значимой она станет для формата с обязательными разрядами, и там молча перебьёт просьбу вызывающего: `Display(2.0m, "0.00")` вернёт `"2"`, а не `"2.00"`. Сегодня таких вызывающих нет.
- **Пять fan-out-методов `CompositePrinterService` повторяют одни и те же четыре строки**, включая обязательный снимок `var printers = _printers`. Шестой метод, написанный без снимка, вернёт гонку, и поймает это только ручной grep. Схлопывается в `FanOutAsync(Func<EscPosPrinterService, Task<bool>>)`. Ни одна из оставшихся задач шестой fan-out не добавляет, поэтому сейчас это чистый churn.
- **Тест на разметку и тест на валюту должны смотреть в разные места.** `BuildFrame` набивает колонки и по устройству не может подставить символ валюты, поэтому утверждение `DoesNotContain("$")` против него сторожит структурную невозможность, а не дефект. Исторический доллар жил в `Money`, и проверять надо `BuildItemFrame`/`BuildTotalFrame`. Записано не как долг, а как урок: при вынесении шва легко перенести тест туда, где проверяемое поведение уже недостижимо, — и красным это не станет никогда.
- **Проверка дисплея не может назвать причину, а пробная печать может.** `TestPrint` обходит композит именно затем, чтобы `ex.Message` доехал до баннера. `CheckDisplay` зовёт `VfdDisplayService`, который ловит всё и пишет причину в `Console` — в оконном приложении это никуда. Наружу выходит только `false`, и таймаут неотличим от занятого порта или мёртвого драйвера. Половина цели «сделать проверку выполнимой на точке» для дисплея не достигнута. Лечится вместе с долговременным логом в батче D.
- **Повторные нажатия по зависшему порту утекают потоком и дескриптором за клик.** `Task.WhenAny` отпускает кнопку через 3 с, но брошенный `SendNowAsync` остаётся заблокированным на пуловом потоке с неосвобождённым `SerialPort`. Каждое следующее нажатие строит **новую** службу с пустой очередью, то есть не встаёт за первой. Кассу не роняет.
- **`ComboBox` кодовой страницы в карточке дисплея обрежется**: бюджет 608 px минус кнопка ~186 даёт ~132 полезных на колонку против ~220 требуемых. Выбор из трёх похоже начинающихся строк по обрезку читается плохо.
- **`SendViaCom` зашивает 9600 бод** и на принтер это не настраивается нигде. COM-принтер на другой скорости провалит пробную печать по причине, которую баннер не объяснит. Предсуществующее, но для приёмки на точке держать в голове.
- **`ConnectionType` в `PrinterConfigViewModel` не nullable, и число вне диапазона ломает строку настроек молча.** Task 13 сделал честным путь печати, но само значение по-прежнему доезжает до экрана: `ConnectionTypes` отдаёт `Enum.GetValues`, `SelectingItemsControl` не находит `(PrinterConnectionType)7` среди них и пишет обратно `null` в не-nullable поле, а `IsLan` и `IsUsbOrCom` оба становятся `false` — то есть скрываются **оба** редактора адреса. Строка показывает пустой выпадающий список типа и вообще никакого поля адреса. Соседний `SelectedCodePage` от этого защищён nullable-типом с комментарием, описывающим ровно эту механику; здесь урок не применён. Нормализовать значение в `SettingsService.Load` при этом **не надо**: безопасной цели нет — `None` в enum отсутствует, `None = 0` перенумерует `USB` и молча переинтерпретирует каждый существующий `settings.json`, а приведение к USB/COM/LAN стёрло бы саму цифру из диагностики.
- **Результат печати на главном пути продажи выбрасывается.** `PosViewModel.cs:2318` — `await _printerService.PrintReceiptAsync(...)` без присваивания. Возвращаемый `bool`, ради честности которого сделана половина батча, сурфейсится только на кнопке повторной печати (`:1822`). На завершении продажи единственный сигнал — полоска статуса принтера.
- **Сообщение `NotSupportedException` — единственная рукописная строка на этом экране, и она английская.** Баннер читается как «<локализованный префикс> Unknown printer connection type 7. Check ConnectionType in settings.json». Кассир на таджикской или узбекской кассе получает полупереведённую фразу с именем файла, до которого он не дотянется.
- **`_overallStatus` и `_status` остались без барьеров.** Task 7 закрыл только `_printers`. Радиус мал: `IPrinterService.Status` не читается в `src/` нигде, единственный потребитель — обработчик `StatusChanged`, который маршалит через `Dispatcher.UIThread.Post`. Худший случай — устаревшая надпись, не падение.
