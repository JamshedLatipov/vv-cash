...
> **Задача выполнена; листинг ниже устарел в одном месте.** Он писался раньше, чем
> ревью Task 6 завело перегрузку `BuildSaleReceipt(EscPosCodePage, SaleReceiptData,
> ReceiptTemplate?)` и запретило раскладывать запись на десять позиционных
> аргументов — тот приём уже однажды молча терял `sale.QueueNumber`, и запрет
> задокументирован прямо в `SaleReceiptData` и покрыт тестом.
>
> Буквальный код шага 4 ниже воскрешает именно этот приём. Фактически сделано иначе:
> заведены две перегрузки `BuildConfiguredSaleReceipt` — по списку позиций и по
> записи, — а `PrintKitchenOrderAsync` зовёт вторую через
> `sale with { QueueNumber = queueNumber }`. Байты при этом те же.
>
> Урок на будущее: план, написанный целиком заранее, не знает о решениях, которые
> примет его собственное ревью. Расхождение такого рода — норма, и его надо ловить
> сверкой с фактическим кодом, а не доверием к тексту.
...
> **Задача выполнена; код шага 5 в листинге ниже содержит ошибку.** Он делает
> проверку пустой подстановки для `qr` и `barcode` ВНУТРИ `switch`, то есть уже
> после безусловного пролога с выравниванием. При пустых данных `return` внутри
> ветки оставляет висячий `AlignOp` — смену выравнивания без текста, сдвигающую
> байты. Проверено: список операций получался из одной операции, голого `AlignOp`.
>
> Тест `Renderer_DropsAQrBlock_WhenItsDataResolvesEmpty` из этого же листинга
> ошибку НЕ ловит: он проверяет отсутствие `QrOp`, а висячее выравнивание — не
> `QrOp`. С ошибочным кодом все семь тестов раздела зелёные.
>
> Фактически сделано правильно: проверка для `qr` и `barcode` вынесена туда же, где
> она стоит для текстового блока — до пролога, — и дублирующий `AlignOp` из веток
> убран. На успешном пути байты те же.
