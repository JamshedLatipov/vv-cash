...
**Если упал — не трогайте фикстуру.** Она и есть эталон. Посмотрите, чем именно разошлись байты:

```bash
python -c "
a=open('tests/VvCash.Tests/Fixtures/sale-receipt-default.bin','rb').read()
b=open('build/verify-tests/Fixtures/sale-receipt-default.bin','rb').read()
print(a.decode('cp866','replace'));print('---');print(b.decode('cp866','replace'))"
```
