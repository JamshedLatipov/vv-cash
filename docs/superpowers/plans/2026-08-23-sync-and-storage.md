...
- **Файлы `Assets/i18n/*.json` — UTF-8 с BOM и CRLF.** Все пять. Правка, снявшая BOM, ломает загрузку локали молча. Проверять побайтово до и после.
...
```
1,000000000000001 -> REAL читает 1,0
12345678901234,56 -> REAL читает 12345678901234,6
```
...
- **Тест конкурентной инициализации вероятностный.** Если он начнёт флакать — удалить и записать сюда, а не глушить повторами.
