...
- **`& ./run-tests.ps1` нельзя перенаправлять под Windows PowerShell 5.1.** Скрипт внутри ставит `$ErrorActionPreference = 'Stop'`, а перенаправление stderr нативного exe оборачивает каждую строку в `NativeCommandError` — вызывающий обрывается на первом же упавшем тесте, то есть ровно в том случае, который мутационный обход и должен наблюдать. Обнаружено финальным обходом: первая попытка умерла на середине и оставила дерево грязным. Перенаправлять внутри `cmd.exe`, либо читать вывод без перенаправления.
...
- **BOM в этом репозитории — по файлу, а не по правилу.** Измерено 2026-08-23, а не предположено:
  ...
  Концы строк — **почти** везде CRLF. Чисто-LF файлов **десять**, измерено финальным обходом: `Models/PhoneFormat.cs`, `Services/ProductImageLoader.cs`, `Services/Update/{IAppVersionProvider,IInstallerLauncher,IUpdateService,UpdateInfo}.cs`, `ViewModels/UpdateViewModel.cs`, `tests/{AppVersionProviderTest,PhoneFormatTest,UpdateViewModelTest}.cs`. Все десять существовали до батча. Правя любой, сохранять LF, а не «чинить» на CRLF — это раздует дифф на весь файл. Первые две редакции плана называли сперва ноль таких файлов, потом четыре.
...
- **`git checkout --` для восстановления файла безопасен, вопреки опасению.** Финальный обход предположил, что `core.autocrlf=true` при `.gitattributes`, закрепляющем только `*.sh`, перепишет чисто-LF файл на CRLF, а `git status` при этом останется чистым. Проверено прямым прогоном: `ProductImageLoader.cs` пережил checkout как `bareLF 101, CRLF 0`, байт в байт. `git ls-files --eol` даёт `i/lf w/lf attr/` — git его не конвертирует. Гипотеза не подтвердилась, записано, чтобы её не выдвигали заново.
...
**Абсолютных чисел «сколько тестов должно пройти» в шагах нет намеренно.** Первая редакция
плана их проставляла, и они устарели уже к третьей задаче: каждая задача добавляет тесты, а
задачи 5-7 отменены вместе со снятием находки #6. Числа в тексте плана после этого вводят в
заблуждение сильнее, чем помогают. Правило: перед задачей запомнить текущий итог, после —
убедиться, что он вырос ровно на число добавленных тестов. Расхождение — повод остановиться
и разобраться, а не подогнать ожидание под факт.
...
Ожидание: прежний итог плюс добавленные этой задачей тесты. Поведение не менялось, ни один тест не должен сдвинуться.
...
Прогнать: `& ./run-tests.ps1`. Ожидание: прежний итог плюс добавленные этой задачей тесты.
...
Ожидание: прежний итог плюс добавленные этой задачей тесты.
...
## Task 5: Сверка применяется — ОТМЕНЕНА

> **Отменена 2026-08-23 вместе со снятием находки #6.** `GET /cashes/remain/` не
> отдаёт `product_id` (`cash_repo.go:152`, тег `json:"-"`), клиентского обхода нет.
> Задача (применение сверки) не выполняется. Обвязка задач 3 и 4 остаётся в дереве, ничем не
> вызывается. Подробности и условия оживления — во врезке «Проблема 1» спеки.
...
## Task 6: Часовая каденция — ОТМЕНЕНА

> **Отменена 2026-08-23 вместе со снятием находки #6.** `GET /cashes/remain/` не
> отдаёт `product_id` (`cash_repo.go:152`, тег `json:"-"`), клиентского обхода нет.
> Задача (часовая каденция) не выполняется. Обвязка задач 3 и 4 остаётся в дереве, ничем не
> вызывается. Подробности и условия оживления — во врезке «Проблема 1» спеки.
...
## Task 7: Плашка «нет по учёту» — ОТМЕНЕНА

> **Отменена 2026-08-23 вместе со снятием находки #6.** `GET /cashes/remain/` не
> отдаёт `product_id` (`cash_repo.go:152`, тег `json:"-"`), клиентского обхода нет.
> Задача (плашка «нет по учёту») не выполняется. Обвязка задач 3 и 4 остаётся в дереве, ничем не
> вызывается. Подробности и условия оживления — во врезке «Проблема 1» спеки.
...
Первый столбец — **номер задачи**, а не находки. Задачи 3 и 4 — это спящий код снятой находки #6;
он остаётся в дереве и его сторожа проверяются наравне с остальными.

| Задача | Откат | Обязано покраснеть |
|---|---|---|
...
| 7 | убрать блок `while (_map.Count > _capacity)` из `GetOrAdd` | все **четыре** теста `ProductImageCacheTest` |
...
- **Дублирование арифметики скидки — тройное, а не двойное, и у него есть дом.** Ревью Task 10
  указало, что `QuoteLineResolver.Resolve` уже владеет операцией «найти строку котировки для
  этой позиции по product id» — для чековой ветки — и отвечает на «какая скидка» **третьим**
  способом: возвращает `DiscountPercent` и `UnitPrice`, а `DiscountAmount` не трогает вовсе.
  Вынести стоит туда: `QuoteLineResolver.PerUnitDiscount(QuoteLineResult?)` или
  `StampOnto(CartItem, QuoteLineResult?)`. Переезжает около шести строк, нового слоя не
  появляется. Не сделано здесь: требует `ExchangeViewModel.cs`, который вне границ Task 10.
- **`QuoteSource` парсится и не читается ничем.** Ноль совпадений по `src/`. Сервер присылает
  имя акции или карты, породившей скидку, а бейдж в корзине может сказать только «−10%» и
  никогда «Летняя акция». Чтобы это использовать, нужно поле в `CartItem` — файл вне границ.
- **Итог строки на экране расходится не между экранами, а между экраном и чеком.** Первая
  редакция этой записи утверждала, что касса и обмен считают итог строки по-разному. Неверно, и
  записано было со слов ревью Task 10 без проверки: `ExchangeWindow.axaml:258` биндит тот же
  `CartItem.LineFinalTotal`, что и касса, то есть `LineTotal − LineDiscount` без округления.
  Путь через `MoneyPolicy.Round` — это `ExchangeViewModel.IssuedLineFinalTotal:711`, и его
  единственный вызывающий — `:858`, сборка `ReturnReceiptLine`. Расхождение экран-против-чека, и
  оно одинаково есть на обоих экранах. Поймано ревью Task 11.

  Практическая цена почти нулевая: `MoneyPolicy.Scale` по умолчанию 2, оба формата печатают два
  знака, деление на единицу даёт только подкопеечные крошки, которые формат скрывает. Видимым
  становится при `Scale = 0` — правдоподобно для UZS и KZT, — где строка чека округлится до целых,
  а экран продолжит показывать `.50`.
- **Бейдж процента в корзине POS показывает каталожную скидку, а не котировочную.**
  `PosView.axaml:446-449` биндит `Product.DiscountPercent` по признаку `IsNotNull`, тогда как
  `ExchangeWindow.axaml:233-235` биндит `QuotedDiscountPercent` по `HasLineDiscount`. Следствие:
  `CartItem.QuotedDiscountPercent`, который Task 10 научился заполнять, в кассе **не читает никто**
  — во всём дереве его единственный потребитель это разметка обмена.

  Прямая замена не бесплатна: офлайновая корзина с каталожной акцией потеряет бейдж, потому что
  котировки там нет. Честное решение — показывать котировочный процент, когда котировка есть, и
  каталожный, когда её нет. Разметкой это не выражается: нужно составное условие, то есть новое
  вычисляемое свойство на `CartItem`, а этот файл был вне границ Task 11. Отдельной задачей.
- **Картинки категорий декодируются в родном разрешении и не кэшируются вовсе.**
  `PosViewModel.cs:1080` делает `new Bitmap(ms)` на отдельном пути, мимо `ProductImageLoader`
  и мимо его границы. Тот же класс проблемы, что Task 12 закрыла для товаров: `ImagePath`
  предпочитает полноразмерное изображение, а `thumb` берёт только когда первого нет, поэтому
  фотография с телефона декодируется целиком. Категорий немного, так что это не утечка того
  же масштаба — но пиковая память и время декодирования те же. Вне границ Task 12.
- **Высота декодированной картинки не ограничена.** `DecodeToWidth(ms, 256)` фиксирует ширину;
  очень узкое и очень длинное изображение всё ещё даст большой битмап. Записано в комментарии
  у `CacheCapacity` честно, а не замолчано. Лечится `DecodeToHeight` или проверкой сторон,
  если такие картинки в каталогах вообще встретятся.
- **`CreditTerms` всё ещё можно собрать наполовину — через инициализатор объекта.**
  `new CreditTerms { Limit = 500m }` компилируется: у `readonly record struct` есть неявный
  беспараметрический конструктор, а позиционные члены — `{ get; init; }`. Измерено: на долге
  400 и реальном балансе −400 такая запись **открывает гейт**, ровно как раньше открывал
  пропущенный `currentBalance`.

  Оставлено сознательно. Дыра ушла с проторённого пути: позиционный конструктор — очевидный
  способ построить тип, и он делает пропуск ошибкой компиляции. Запечатать можно, убрав
  позиционную форму и объявив члены как `{ get; }` без `init`, — ценой `with`-выражений и
  однострочного объявления. Соотношение не в пользу правки.
