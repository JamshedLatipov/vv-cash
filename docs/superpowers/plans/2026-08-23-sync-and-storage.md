...
- **Итог строки два экрана считают по-разному.** Обмен берёт серверный `final_line_total` и
  прогоняет через `MoneyPolicy.Round` (`ExchangeViewModel.cs:713`); касса выводит
  `LineFinalTotal = LineTotal - LineDiscount` вообще без округления. Расходятся, когда
  серверный итог не равен в точности `unit_price × qty − discount_amount`, и когда деление на
  единицу не завершается: `10m/3m*3m` даёт `9.9999…9`, а не `10m`. Скидку на единицу Task 10
  выровняла, итог строки — нет. Разметка Task 11 не должна цементировать допущение, что
  итоги сошлись.
