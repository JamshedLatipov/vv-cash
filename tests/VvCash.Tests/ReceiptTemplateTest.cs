...
public class ReceiptTemplateTest
{
    ...
    public void Parse_DropsAnUnknownBlockType_AndKeepsTheRest()
    {
        ...
        //
        // Отбрасывание сопровождается строкой в лог с самим type — иначе
        // опечатка вида "field" вместо "fields" не оставляет на чеке никакого
        // следа, а серверной валидации, которая ловила бы её раньше, ещё нет.
        // Саму строку здесь не проверяем: раннер не перехватывает
        // Console.WriteLine, а перехват потребовал бы редиректа Console.Out,
        // который сделал бы тест тестом водопровода, а не поведения — тот же
        // довод, что и в
        // OfflineStorageServiceTest.InitializeAsync_WhenARebuildFails_LogsAndCarriesOn.
        // Под тестом остаётся то, что реально видно на чеке: он печатается без
        // "hologram", но с "A" и "B" на месте.
        var json = """
