...
public class CompositePrinterService : IPrinterService
{
    ...
    ///
    /// printerFactory, если передана, читает template сама или не читает вовсе —
    /// параметр не гасится молча, но и не навязывается: своя фабрика полностью
    /// отвечает за то, что получит собранный ею EscPosPrinterService, ровно как
