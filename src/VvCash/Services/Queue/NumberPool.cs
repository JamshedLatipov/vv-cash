...
public class NumberPool : INumberPool
{
    ...
    /// он ничего не решает. «Отстоявшие» номера — это свободные, у которых
    /// ReleasedAtSeq не выше seq − 50, то есть начальный отрезок очереди
    /// свободных, упорядоченной по ReleasedAtSeq; выдавать сначала их по
    /// ReleasedAtSeq, а потом остальные свободные по ReleasedAtSeq — то же
    /// самое, что выдавать все свободные по ReleasedAtSeq. Единственное, что
    /// кулдаун мог изменить, — заставить кассу отобрать живой номер вместо
    /// недавно освобождённого, и именно это в нём и было ошибкой. Обещание
    /// заказчику — «один номер не окажется у двух человек сразу» — новый
    /// порядок держит строже: свободный номер выходит снова только после всех
    /// свежих и всех освобождённых раньше него, а живой не трогается, пока
    /// есть хоть один ничей.</summary>
