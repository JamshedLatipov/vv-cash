...
/// принтера. Источник Bitmap печатает растр из отдельной опции конфига
/// receipt_logo (GS v 0) — см. ReceiptRenderer.ParseLogo. Блок отбрасывается
/// целиком, до пролога (см. ReceiptRenderer.RenderBlock), когда печатать
/// нечего: опция ещё не доехала синхронизацией, пришла битой, объявляет
/// нулевой размер, или Source несёт значение, которого этот enum сегодня не
/// знает (шесть лет опция правилась руками через текстовое поле, и разбор
/// шаблона принимает число вне диапазона enum без проверки).</remarks>
public sealed class LogoBlock : ReceiptBlock
{
    public LogoSource Source { get; set; } = LogoSource.Nv;

    /// <summary>Диапазон 1–255 — то, что несёт однобайтовый параметр n у
    /// FS p n m (n — слот логотипа, m — режим печати; здесь только n). Без
    /// верхнего потолка здесь 1000 приведением к byte стало бы 232: не
    /// отвергнутый вход, а чужой слот на бумаге.</summary>
